...
/**
 * Created by heyanjing on 2017/5/4 17:18.
 */


public class LocatePrint implements Printable {
    private int PAGES = 0;
    private String printStr;     /*      * Graphic指明打印的图形环境；PageFormat指明打印页格式（页面大小以点为计量单位，      * 1点为1英寸的1/72，1英寸为25.4毫米。A4纸大致为595×842点）；page指明页号      */
    ...
    public int print(Graphics gp, PageFormat pf, int page) throws PrinterException {
        ...
        return Printable.PAGE_EXISTS; // 存在打印页时，继续打印工作     }     // 打印内容到指定位置
    }
    public void printContent() {
        printStr = "打印测试内容";// 获取需要打印的目标文本
        if (printStr != null && printStr.length() > 0) // 当打印内容不为空时
        {
            PAGES = 1; // 获取打印总页数             // 指定打印输出格式
